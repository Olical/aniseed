...
If you would like your module to be updated interactively while you work on it, simply wrap the returned table in a call to `module` from `:aniseed.core`. Aniseed will now update the module globally when you load the entire file or buffer.
...
The locals at the time of the module definition are also captured by `core.module`. When you evaluate a form in your module it'll have access to all of the previously defined locals, this allows you to call internal module functions without exposing the globally! In practice, this means we can evaluate the `(core.pr (add 10 20))` line on it's own and it'll know to call the `add` function which is only visible as a local at that time as well as have access to the `core` require.

Be careful though, to redefine one of those locals you'll have to evaluate the entire file again, recapturing the locals at the point of entire module evaluation. Simply evaluating a function hidden within a module won't capture it for future evaluations or expose it to the world.
