...
There's a few projects in `deps` (when you run `make compile` while working on Aniseed itself) which have some files copied into the `lua/aniseed` directory, they're extremely useful, Aniseed relies on them internally but you also have access to them.
