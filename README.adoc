...
This is aimed at people who want to extend Neovim interactively with a Lisp as you would in some _other_ text editor. Code is evaluated interactively as you work, it feels a lot like editing Clojure with https://github.com/Olical/conjure[Conjure].
...
You can continue to add more files (modules) under `fnl/dotfiles` and require them using the `module` macro syntax. Feel free to get creative with it!

== Compiling a plugin ahead of time

https://github.com/Olical/nvim-local-fennel[nvim-local-fennel] and the experimental https://github.com/Olical/conjure-sourcery[conjure-sourcery] should be good examples of Fennel based plugins (I hope so, I wrote them), here's how to start a new one.

We'll start by fetching the `dep.sh` helper script which we'll use to clone and update Aniseed when required. It can be used for repos other than Aniseed but I won't be showing that here.
...
Now let's add a tiny program to `fnl/my-plugin/init.fnl`.
...
We can compile the plugin using `deps/aniseed/scripts/compile.sh` which will write into the `lua` directory, you can commit that output so that users of your plugin don't have to know it was ever written in Fennel in the first place.

Users can call into your plugin however they choose or you can add some sort of startup hook into `plugin/my-plugin.vim` (I'm deeply sorry that a little Vim Script is involved).
